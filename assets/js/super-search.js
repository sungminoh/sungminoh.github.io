  ...
  function handleInput() {
    ...
    if (matchingPosts.length && currentResultHash !== lastSearchResultHash) {
      ...
      searchResultsEl.innerHTML = matchingPosts.map(function (post) {
        ...
        return '<li><a href="' + post.link + '">' + '<span class="search__result-title">' + post.title + '</span>' + '<span class="search__result-date">' + d.toUTCString().replace(/.*(\d{2})\s+(\w{3})\s+(\d{4}).*/,'$2 $1, $3') + '</span></a></li><div style="clear: both;"></div>';
      }).join('');
    }
